...
lapply(
  ...
  FUN = function(i,
    ...
    df <- as.data.frame(df)
    object <- lm(y ~ ., data = df)
    mvn <- PCorMC(MC(object, R = R, type = "mvn"))
    adf <- PCorMC(MC(object, R = R, type = "adf"))
    hc0 <- PCorMC(MC(object, R = R, type = "hc0"))
    hc1 <- PCorMC(MC(object, R = R, type = "hc1"))
    hc2 <- PCorMC(MC(object, R = R, type = "hc2"))
    hc3 <- PCorMC(MC(object, R = R, type = "hc3"))
    hc4 <- PCorMC(MC(object, R = R, type = "hc4"))
    hc4m <- PCorMC(MC(object, R = R, type = "hc4m"))
    hc5 <- PCorMC(MC(object, R = R, type = "hc5"))
  ...
  text = "test-external-betaMC-p-cor-mc",
  tol = 0.01,
  n = 100000L,
  p = 2,
  beta = 0.5,
