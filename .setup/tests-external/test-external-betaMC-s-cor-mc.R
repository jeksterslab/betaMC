...
lapply(
  ...
  FUN = function(i,
    ...
    df <- as.data.frame(df)
    object <- lm(y ~ ., data = df)
    mvn <- SCorMC(MC(object, R = R, type = "mvn"))
    adf <- SCorMC(MC(object, R = R, type = "adf"))
    hc0 <- SCorMC(MC(object, R = R, type = "hc0"))
    hc1 <- SCorMC(MC(object, R = R, type = "hc1"))
    hc2 <- SCorMC(MC(object, R = R, type = "hc2"))
    hc3 <- SCorMC(MC(object, R = R, type = "hc3"))
    hc4 <- SCorMC(MC(object, R = R, type = "hc4"))
    hc4m <- SCorMC(MC(object, R = R, type = "hc4m"))
    hc5 <- SCorMC(MC(object, R = R, type = "hc5"))
  ...
  text = "test-external-betaMC-s-cor-mc",
  tol = 0.01,
  n = 100000L,
  p = 2,
  beta = 0.5,
