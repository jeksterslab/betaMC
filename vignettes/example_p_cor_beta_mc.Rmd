---
title: "betaMC: Example Using the PCorBetaMC Function"
author: "Ivan Jacob Agaloos Pesigan"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{betaMC: Example Using the PCorBetaMC Function}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

Confidence intervals for
partial correlation coefficients
(semipartial correlation, squared semipartial correlation, and squared partial correlation)
are generated using
the `PCorBetaMC()` function from the `betaMC` package.
In this example,
we use the data set and the model used in
[betaMC: Example Using the BetaMC Function](example-beta-mc.html).

```{r}
#| message = FALSE
library(betaMC)
```

```{r}
#| echo = FALSE
if (!exists("nas1982")) {
  try(
    data(
      "nas1982",
      package = "betaMC"
    ),
    silent = TRUE
  )
}
df <- nas1982
```

```{r}
#| eval = FALSE
df <- betaMC::nas1982
```

## Fit the regression model using the `lm()` function.

```{r}
object <- lm(QUALITY ~ NARTIC + PCTGRT + PCTSUPP, data = df)
```

## Estimate the standardized regression slopes and the corresponding sampling covariance matrix.

#### Multivariate Normal-Theory Approach

```{r}
std_mvn <- BetaMC(object, type = "mvn")
```

#### Asymptotic Distribution-Free Approach

```{r}
std_adf <- BetaMC(object, type = "adf")
```

#### HC3

```{r}
std_hc3 <- BetaMC(object, type = "hc3")
```

## Estimate the multiple correlation coefficients (R-squared and adjusted R-squared) and the corresponding sampling covariance matrix.

```{r}
mvn <- PCorBetaMC(std_mvn)
adf <- PCorBetaMC(std_adf)
hc3 <- PCorBetaMC(std_hc3)
```

## Methods

### summary

Summary of the results of `PCorBetaMC()`.

```{r}
summary(mvn)
summary(adf)
summary(hc3)
```

### coef

Calculate partial correlation coefficients.

```{r}
coef(mvn)
coef(adf)
coef(hc3)
```

### vcov

Calculate the sampling covariance matrix of partial correlation coefficients.

```{r}
vcov(mvn)
vcov(adf)
vcov(hc3)
```

### confint

Generate confidence intervals for partial correlation coefficients.

```{r}
confint(mvn, level = 0.95)
confint(adf, level = 0.95)
confint(hc3, level = 0.95)
```

## References

National Research Council. (1982).
*An assessment of research-doctorate programs in the United States: Social and behavioral sciences*.
https://doi.org/10.17226/9781.
Reproduced with permission from the National Academy of Sciences,
Courtesy of the National Academies Press, Washington, D.C.
